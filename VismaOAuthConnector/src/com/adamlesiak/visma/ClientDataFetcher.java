...
public class ClientDataFetcher {
	...
	public void requestAccessToken(String tokenEndpoint) throws IOException {
        ...
        StringBuffer accesTokenBuffer = new StringBuffer();
        ...
        Reader inputReader = new BufferedReader(new InputStreamReader(conn.getInputStream(), "UTF-8"));
        for (int row; (row = inputReader.read()) >= 0;) {
        	accesTokenBuffer.append((char) row);
        }
		
	    accessToken = accesTokenBuffer.toString();
